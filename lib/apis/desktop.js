...
function get_process(id, callback) {
  processes.forEach(function(item) {
    if (item.id === id) {
      callback(item);
    }
...
var apps = {
  open: function(name) {
    ...
    //apps[i];
    $(".windows-container").append("<div class='window' id='" + newapp.id + "' style='display: none;'><div class='window-bar'><div></div><div>" + newapp.name + "</div><div><div class='window-close'>&times;</div></div></div><div class='window-content'><iframe src='/lib/php/app_template?app=" + newapp.name + "' frameborder=0 allowtransparency></iframe></div></div>");
    $(".window").draggable({
      handle: ".window-bar",
      containment: "parent"
    });
