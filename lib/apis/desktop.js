    ...
    if ($(".window[data-title='" + name + "']").length !== 0 && newinstance === false) {
      ...
      network.request(
        ...
        function(response) {
          ...
          $(".menubar-menu li").on("click", function() {
            var id = $(this).parent().parent().parent().parent().attr("id").replace("menu-");
            var func = $(this).attr("data-function");
            document.querySelector(".window #" + id + " iframe").contentWindow.func();
          });
        },
        function(error) {
  ...
  close: function(id) {
    ...
    get_process(id, function(process) {
      if ($(".window[data-title='" + process.name + "']").length === 0) {
        $(".app-item[tooltip='" + process.name + "']").removeClass("opening").removeClass("open").html("");
      }
      process.kill();
    });
  },
...
var options = {
  ...
  test: {
    label: "UI Gallery",
    onClick: "apps.open(&quot;UI Gallery&quot;,true)",
    divider: true
  },
  logout: {
    label: "Logout",
