class Menu {
	constructor(label, index) {
		...
		if ($('.menubar-menu[data-label=\'' + this.label + '\']').length === 0) {
			...
			let newmenu = '<div class=\'menubar-menu-container\' data-label=\'' + this.label + '\'><div class=\'menubar-label\'>' + this.label + '</div><div class=\'menubar-menu\'><ul></ul></div></div>';
			if (this.index) {
	...
	add(item, shortcut, callback) {
		...
		setTimeout(function () {
			...
			let menu = main.$('.app-menu#menu-' + app.id + ' .menubar-menu-container[data-label=\'' + instance.label + '\'] .menubar-menu ul');
	...
	remove() {
		main.$('.app-menu#menu-' + app.id + ' .menubar-menu-container[data-label=\'' + this.label + '\']').remove();
	}

	divider() {
		let that = this;
		setTimeout(function () {
			let menu = main.$('.app-menu#menu-' + app.id + ' .menubar-menu-container[data-label=\'' + that.label + '\'] .menubar-menu ul');
			menu.append('<hr/>');
		}, 500);
	}

	clear() {
		main.$('.app-menu#menu-' + app.id + ' .menubar-menu-container[data-label=\'' + this.label + '\'] .menubar-menu ul').html('');
	}
}
