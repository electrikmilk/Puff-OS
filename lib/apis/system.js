...
apps = {
	...
	open: async function (name, newinstance = false, params) {
		if ($('.window[data-title=\'' + name + '\']').length !== 0 && newinstance === false) {
			...
			let new_app = await new Process(name, 'app');
			checkApp = setInterval(function () {
				apps.windows().windows.each(function () {
					if (this.id === new_app.id) {
						main.log('Created window for \'' + new_app.name + '-' + new_app.id + '\'');
						apps.status(new_app.id);
						clearInterval(checkApp);
					} else {
						apps.unresponsive(new_app.id);
						main.log('\'' + new_app.name + '-' + new_app.id + '\' unresponsive', 'warn');
						// watchdog should take it from here...
					}
				});
				if (!open_apps.includes(new_app)) {
					apps.unresponsive(new_app.id);
					main.log('\'' + new_app.name + '-' + new_app.id + '\' unresponsive', 'warn');
					// watchdog should take it from here...
				} else {
			...
			network.newRequest({
				...
				error: function (error) {
					...
					apps.open('Dialog', true, {
						title: 'App \'' + name + '\' failed to launch',
						message: 'Unable to create window.',
						cancel: 'false'
					});
				}
			});
			if (desktop !== null && !(params && params.url)) {
				network.request(
					'lib/server', 'POST', {
	...
	applet: function (name, data) {
		if (!process_exists(name + ' Applet')) {
			let applet = new Process(name + ' Applet', 'applet');
			if (data.type === 'popup') {
				let icon = 'icon/32.png';
				if (data.icon) {
...
class Process {
	constructor(name, type, interval) {
		...
		console.log('[system] Started process \'' + name + '\'.', 'success', this);
	}

	// attach an element to this process
	attach(selector) {
...
$(async function () {
	...
	if (performance.memory) {
		...
		memorymonitor = new Process('memoryd', 'demon', memorycheck);
	...
	let watchdog = await new Process('watchdogd', 'demon', patrol);
	await sleep(3);
	system.ready();
});