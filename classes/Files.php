...
class Files
{
						...
						if ($handle = opendir($path)) {
							while (false !== ($entry = readdir($handle))) {
								if ($entry != "." && $entry != ".." && $entry[0] !== ".") {
									...
									elseif ($orderby === "az") $files[$entry] = $this->info("$entry");
									else {
	...
	private function transfer($action, $path, $newpath, $force)
	{ // TODO: make it so you could write a funciton like this copy("file","directory"), not copy("file","directory/file");
		if (is_file($path)) {
			if ($action === "copy") {
				if (!$newpath) {
					$info = pathinfo($path);
	...
	private function copy_dir(string $path, string $newpath = null)
	{ // not properly tested yet, not sure if this will work correctly
		$dir = opendir($path);
		@mkdir($newpath);
